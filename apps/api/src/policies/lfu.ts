...
export class LFUPolicy extends BasePolicy {
  ...
  protected _lowestHitCount: number = 0;

  constructor(driver: Driver) {
    super(Policy.LFU, driver);
    ...
    tracer.startActiveSpan(
      ...
      (span) => {
        ...
        if (this._keyOrderMap.get(hits)?.length === 0) {
          ...
          if (this._lowestHitCount === hits) this._lowestHitCount = min(Array.from(this._keyOrderMap.keys())) || 0;
        }
  ...
  hit(hash: string): void {
    tracer.startActiveSpan(
      ...
      (span) => {
        ...
        if (this._lowestHitCount < newHitCount) this._lowestHitCount = newHitCount;
  ...
  evict(): string | null {
    return tracer.startActiveSpan(
      ...
      (span) => {
        const hash = this._keyOrderMap.get(this._lowestHitCount)?.pop();
        if (hash === undefined) {
          span.end();
          return null;
        }
        ...
        if (this._keyOrderMap.get(this._lowestHitCount)?.length === 0) {
          this._keyOrderMap.delete(this._lowestHitCount);
          this._lowestHitCount = min(Array.from(this._keyOrderMap.keys())) || 0;
        }
