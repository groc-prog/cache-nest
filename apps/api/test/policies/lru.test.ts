...
import { LRUNode, LRUPolicy } from '@/policies/lru';
...
class TestLRUPolicy extends LRUPolicy {
  ...
  setMockedCacheKeyMap(map: Record<string, LRUNode>) {
    Object.keys(map).forEach((hash) => {
      this._cacheKeyMap.set(hash, map[hash] as LRUNode);
    });
  }

  setMockedMostRecentlyUsedNode(node: LRUNode) {
    this._mostRecentlyUsed = node;
  }

  setMockedLeastRecentlyUsedNode(node: LRUNode) {
    this._leastRecentlyUsed = node;
  }

  setMockedNodes() {
    const node1 = new LRUNode('i318rbr23ht2tk2');
    const node2 = new LRUNode('kjsdu238dh9aeb2');
    const node3 = new LRUNode('8hisa6rwe2t4n');
...
describe('LRUPolicy', () => {
  ...
  describe('.track()', () => {
    ...
    it('should do nothing if the hash is already been tracked', () => {
      ...
      expect(setSpy).toHaveBeenNthCalledWith(1, 'i318rbr23ht2tk2', expect.any(LRUNode));
