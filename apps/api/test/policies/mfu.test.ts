...
class TestMFUPolicy extends MFUPolicy {
  get keyOrderMap() {
    return this._keyOrderMap;
  }

  get highestHitCount() {
    return this._highestHitCount;
  }
  ...
  setHighestHitCount(count: number) {
    this._highestHitCount = count;
  }
  ...
  setMockedNodes() {
    ...
    this._highestHitCount = 2;
  }
}

describe('MFUPolicy', () => {
  let policy: TestMFUPolicy;

  beforeEach(() => {
    jest.clearAllMocks();
    policy = new TestMFUPolicy(Driver.MEMORY);
  });
  ...
  describe('.stopTracking()', () => {
    ...
    it('should remove the key from the key order map if no more hashes share the hit count', () => {
      ...
      expect(policy.highestHitCount).toBe(0);
    });
  ...
  describe('.hit()', () => {
    it('should update the highest hit count, key order and hash map', () => {
      ...
      expect(policy.highestHitCount).toBe(3);
    });
  ...
  describe('.evict()', () => {
    it('should remove the hash with the most hits', () => {
      const clearTTLSpy = spyOn(policy, 'clearTTL');
      ...
      expect(clearTTLSpy).toHaveBeenCalledWith('8hisa6rwe2t4n');
      expect(hash).toBe('8hisa6rwe2t4n');
      expect(policy.cacheKeyMap.size).toBe(2);
      expect(policy.keyOrderMap.size).toBe(2);
      expect(policy.highestHitCount).toBe(2);
    });

    it('should update the highest hit count correctly', () => {
      const clearTTLSpy = spyOn(policy, 'clearTTL');
      ...
      hash = policy.evict();
      expect(hash).toBe('kjsdu238dh9aeb2');
      expect(policy.cacheKeyMap.size).toBe(1);
      expect(policy.keyOrderMap.size).toBe(1);
      expect(policy.highestHitCount).toBe(0);
    });
